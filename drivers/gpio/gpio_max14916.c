...
static int gpio_max14916_port_set_masked_raw(const struct device *dev, gpio_port_pins_t mask,
...
static int gpio_max14916_config_diag(const struct device *dev)
{
	...
	ret = max149x6_reg_transceive(dev, MAX14916_OW_ON_EN_REG, data->chan_en.ow_on_en, NULL,
				      MAX149x6_WRITE);
	if (ret < 0) {
		return ret;
	}

	ret = max149x6_reg_transceive(dev, MAX14916_OW_OFF_EN_REG, data->chan_en.ow_off_en, NULL,
				      MAX149x6_WRITE);
	if (ret < 0) {
		return ret;
	}

	ret = max149x6_reg_transceive(dev, MAX14916_SHT_VDD_EN_REG, data->chan_en.sht_vdd_en, NULL,
				      MAX149x6_WRITE);
	if (ret < 0) {
		return ret;
	}
